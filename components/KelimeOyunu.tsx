...
export default function KelimeOyunu() {
  ...
  const [mod, setMod] = useState(null); // "de_to_tr" | "tr_to_de"
  ...
  const oyunuBaslat = useCallback((secilenTema, secilenMod) => {
    const kelimeler = shuffle(KELIMELER[secilenTema].kelimeler).slice(0, 15);
    const hazirSorular = kelimeler.map(k => {
        ...
        <div style={{ maxWidth: 600, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 32 }}>
            ...
            <p style={{ color: "#a78bfa", marginTop: 8, fontSize: 15 }}>
              Goethe A1 — 360 kelime, 12 tema
            </p>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {Object.entries(KELIMELER).map(([key, val], i) => (
              <button
                key={key}
                onClick={() => setTema(key)}
                style={{
    ...
    return (
      <div style={{
        ...
        <div style={{ maxWidth: 400, width: "100%", textAlign: "center" }}>
          ...
          <p style={{ color: "#a78bfa", fontSize: 14, marginBottom: 32 }}>
            Hangi yönde çalışmak istersin?
          </p>

          {[
            { id: "de_to_tr", icon: "🇩🇪→🇹🇷", label: "Almanca → Türkçe", desc: "Almanca kelimeyi gör, Türkçesini bul" },
            { id: "tr_to_de", icon: "🇹🇷→🇩🇪", label: "Türkçe → Almanca", desc: "Türkçe anlamı gör, Almancasını bul" }
          ].map(m => (
            <button
              key={m.id}
          ...
          <button
            ...
            style={{ background: "none", border: "none", color: "#666", cursor: "pointer", fontSize: 14, marginTop: 8 }}
          >
            ← Geri dön
          </button>
        </div>
        ...
        <div style={{ maxWidth: 380, width: "100%", textAlign: "center" }}>
          ...
          <div style={{
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16, marginBottom: 16 }}>
              {[
                { label: "Skor", val: skor, color: "#a78bfa" },
                { label: "Doğru", val: dogru, color: "#34d399" },
                { label: "Yanlış", val: yanlis, color: "#f87171" }
              ].map(s => (
                <div key={s.label}>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            ...
            <button
              onClick={() => { setTema(null); setMod(null); }}
              style={{
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 520, margin: "0 auto" }}>
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontSize: 11, color: "#666", marginBottom: 12, textTransform: "uppercase", letterSpacing: 2 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          {suankiSoru.secenekler.map((s, i) => {
            ...
            if (secilenCevap) {
              ...
              } else if (s === secilenCevap && s !== suankiSoru.dogru) {
            ...
            return (
              <button
                ...
                onMouseEnter={e => { if (!secilenCevap) e.currentTarget.style.background = "rgba(167,139,250,0.2)"; }}
                onMouseLeave={e => { if (!secilenCevap) e.currentTarget.style.background = "rgba(255,255,255,0.07)"; }}
              >
                {s}
              </button>
            );
